export class LiftSystem {
  ...
  playDoorSound() {
    if (this.doorSound) {
      ...
      this.doorSound
        .play()
        .catch((err) => console.log("Error playing door sound:", err));
    }
  }
  ...
  playArrivalSound() {
    if (this.arrivalSound) {
      ...
      this.arrivalSound
        .play()
        .catch((err) => console.log("Error playing arrival sound:", err));
    }
  }
  ...
  updateLiftPosition(lift) {
    const bottomPosition =
      (this.numFloors - lift.currentFloor) * this.floorHeight;
    ...
    return this.lifts
      ...
      .reduce((nearest, lift) => {
        ...
        const currentDistance = Math.abs(
          lift.currentFloor - pendingRequest.floor,
        );
        const nearestDistance = Math.abs(
          nearest.currentFloor - pendingRequest.floor,
        );
